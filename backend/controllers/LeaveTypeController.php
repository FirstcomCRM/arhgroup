...
use common\models\SearchLeaveType;
...
class LeaveTypeController extends Controller
{
    ...
    public function behaviors()
    {
        ...
        foreach ( $userRoleArray as $uRId => $uRName ){ 
            $permission = UserPermission::find()->where(['controller' => 'LeaveType'])->andWhere(['role_id' => $uRId ])->andWhere(['status' => 1 ])->all();
    ...
    {
        $model = new LeaveType();
        $searchModel = new SearchLeaveType();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);

        if( !empty(Yii::$app->request->get('SearchLeaveType')['name'])) {
                $getLeaveType = $searchModel->SearchLeaveTypeName(Yii::$app->request->get('SearchLeaveType')['name']);

        }else {
                $getLeaveType = LeaveType::find()->where(['status' => 1])->all();
    ...
    {
        $result = LeaveType::find()->where('id > 1')->andWhere(['status' => 1])->all();
        ...
        $filename = "LeaveTypeList-".date("m-d-Y").".xls";
    ...
    public function actionExportPdf() 
    {
        ...
        $dompdf->stream('LeaveTypeList-' . date('m-d-Y'));
    }
